...
export class AchievementsSection {
  ...
  private renderCard(achievement: Achievement, index: number): string {
    ...
    return `
      ...
      >
        <!-- Card -->
          <div class="relative h-full backdrop-blur-2xl bg-white/90 dark:bg-gray-900/90 border-2 border-gray-200/30 dark:border-gray-800/50 rounded-3xl overflow-hidden shadow-2xl hover-lift hover:border-${achievement.platform === 'gfg' ? 'emerald' : achievement.platform === 'leetcode' ? 'orange' : 'cyan'}-500/50 transition-all duration-500">
